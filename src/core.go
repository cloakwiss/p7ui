...
func handleHookClient(p7 *ApplicationState, dataC chan<- HookData, conn net.Conn) {
...
// ---------------------------------------------------------------------------------------------- //
func (p7 *ApplicationState) Launch(dataC chan<- HookData) {
	p7.InjectDLL()
	defer p7.RemoveDLL()
	...
	go func() {
		for {
			...
			go handleHookClient(p7, dataC, conn)
		}
	}()
...
// 	hookListener, err := winio.ListenPipe(p7.InPipeName, pipeCfg)
// 	if err != nil {
// 		p7.Log.Fatal("Failed to create pipe %v", err)
// 	}
// 	defer hookListener.Close()
...
// 			handleHookClient(p7, dataC, conn)
// 		}
// 	}()
